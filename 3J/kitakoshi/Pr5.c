...
void calc(void)
{
	...
	if(check(targetFrom)==ERROR || check(targetTo)==ERROR){
		printf("そのような駅は存在しません。");
		return;
	}
	if(
...
/* どちらの駅が前にあるのかチェック */
int checkStationOrder(STATION a,STATION b)
{
        STATION current_addr;
        current_addr = head;
	...
	int i=-1,j=-1,cnt=0;
        while(i==-1 || j==-1){
                if(strcmp(a.name,current_addr.name)==0)
                        i=cnt;
                if(strcmp(b.name,current_addr.name)==0)
                        j=cnt;
                /* current_addrを更新 */
        }
        return ERROR;
